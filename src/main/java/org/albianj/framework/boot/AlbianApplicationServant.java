...
public class AlbianApplicationServant {
    ...
    public void attachBundle(Class<?> refType,String name,String workFolder,String bundleStartbyType){
        if(isBundleExist(name)){
            ThrowableServant.Instance.throwDisplayException(refType,null,"Repeat startup bundle.",
                    "Bundle -> {0} was starting.",name);
        }
        BundleAttribute bundleAttr = new BundleAttribute(name,workFolder,bundleStartbyType);
        attAttrs.put(name,bundleAttr);
